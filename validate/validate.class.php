...
class Validate
{
    ...
    public function ValidateAJAX($inputValue, $fieldId)
    {
        ...
        switch($fieldId)
        {
            ...
            break;
            // Check whether a gender was selected
            case 'selGender':
    ...
    // validates all form fields on the form submit
    public function ValidatePHP()
    {
        // error flag, becomes 1 when the errors are found.
        $errorsExist = 0;
    ...
    {
        // valid birth year is between 1900 and 2005
        // get the whole date (mm#dd#yyyy)
        $date = explode('#',$value);
        // date cannot be valid if there is no day, month, or year
        if (!$date[0]) return 0;
    ...
    {
        // valid email formats: *@*.*@*.*.*, *.*@*.*, *.*@*.*)
        return (!preg_match('/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}/i', $value)) ? 0 : 1;
    }
    // validate phone
